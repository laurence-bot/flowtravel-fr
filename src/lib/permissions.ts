export type AppRole = "super_admin" | "administrateur" | "agent" | "gestion" | "comptable" | "lecture_seule";
...
const ROUTE_ACCESS: Record<AppRole, string[]> = {
  ...
  agent: [
    // Pas de /pilotage (données financières), pas de /factures, pas de /ops/*
    "/",
    "/app",
    "/contacts",
    "/dossiers",
  ...
  gestion: ["/", "/app", "/pilotage", "/contacts", "/dossiers", "/cotations", "/demandes", "/support", "/mon-espace"],
  comptable: [
    "/",
    "/app",
    "/comptes",
...
export function canWrite(role: AppRole | null): boolean {
  return role === "super_admin" || role === "administrateur" || role === "agent" || role === "gestion";
}
...
export function canWriteIn(
  ...
  if (role === "comptable") {
    return module === "paiements" || module === "comptes" || module === "rapprochement" || module === "import";
  }
  return false;
}
