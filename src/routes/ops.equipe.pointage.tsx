...
import {
  listTimeEntriesAgence, listEmployees, listPlanning, calcHeuresPointees, calcHeuresRealisees,
  TIME_EVENT_LABELS, type Employee, type TimeEntry, type PlanningEntry,
} from "@/lib/hr";
...
function PointagePage() {
  ...
  const ecarts = useMemo(() => employees.filter(e => e.actif).map(emp => {
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader title="Pointage temps réel" description="Présences, historique et écart planifié vs pointé"
          ...
          <tbody>
            {ecarts.map(({ emp, planifie, pointe, ecart }) => (
              <tr key={emp.id} className="border-t">
                ...
                <td className={`px-4 py-2 text-right tabular-nums font-medium ${ecart > 0 ? "text-emerald-600" : ecart < 0 ? "text-red-500" : "text-muted-foreground"}`}>
                  {ecart > 0 ? "+" : ""}{ecart}h
                </td>
              </tr>
            ))}
