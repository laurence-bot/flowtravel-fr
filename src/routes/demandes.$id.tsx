...
function DemandeDetail() {
  ...
  const [historique, setHistorique] = useState<
    Array<{ id: string; description: string; created_at: string; action: string }>
  >([]);
  ...
  useEffect(() => {
    if (demande) {
      ...
      supabase
        ...
        .eq("entity_id", demande.id)
        .order("created_at", { ascending: false })
        .limit(20)
        .then(({ data }) => setHistorique((data ?? []) as any));
    }
  }, [demande]);
      ...
      {historique.length > 0 && (
        <Card className="p-5 space-y-3">
          ...
          <div className="space-y-0">
            {historique.map((h, i) => (
              <div key={h.id} className="flex gap-3 py-2 border-b last:border-b-0">
                <div className="flex flex-col items-center gap-1 pt-1">
                  <div className="w-2 h-2 rounded-full bg-primary/40 shrink-0" />
                  {i < historique.length - 1 && <div className="w-px flex-1 bg-border/50 min-h-[16px]" />}
                </div>
