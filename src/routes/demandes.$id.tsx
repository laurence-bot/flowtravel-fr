...
import {
  DEMANDE_STATUT_LABELS, DEMANDE_CANAL_LABELS,
...
function DemandeDetail() {
    ...
    return (
      <div className="p-8 text-center text-muted-foreground">
        ...
        <Button asChild variant="outline" className="mt-4">
          <Link to="/demandes"><ArrowLeft className="h-4 w-4 mr-2" />Retour</Link>
        </Button>
      </div>
    );
  }
  ...
  const updateStatut = async (statut: DemandeStatut, raison?: string) => {
    ...
    await logAudit({
      userId: user.id, entity: "demande", entityId: demande.id, action: "update",
  ...
  const transformer = async () => {
    ...
    if (!clientId) {
      ...
      const { data: c, error: e1 } = await (supabase as any)
        .from("contacts")
        .insert({
          user_id: user.id, nom: demande.nom_client, type: "client",
          email: demande.email, telephone: demande.telephone,
        })
        .select().single();
      if (e1 || !c) { toast.error(e1?.message ?? "Création client impossible"); return; }
      clientId = c.id;
    }
    ...
    const { data: cot, error: e2 } = await (supabase as any)
      ...
      .select().single();
    if (e2 || !cot) { toast.error(e2?.message ?? "Création cotation impossible"); return; }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabase as any)
      .from("demandes")
      .update({
        statut: "transformee_en_cotation", client_id: clientId,
        dernier_contact_at: new Date().toISOString(),
      })
      .eq("id", demande.id);
    await logAudit({
    ...
    await logAudit({
      userId: user.id, entity: "cotation", entityId: cot.id, action: "create",
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="p-5 space-y-3">
          ...
          {client ? (
            <Button asChild variant="outline" size="sm" className="w-full">
              <Link to="/contacts/$id" params={{ id: client.id }}>Voir la fiche client</Link>
            </Button>
          ) : (
            canWrite && (
              <Button onClick={creerClient} variant="outline" size="sm" className="w-full">
                <UserPlus className="h-4 w-4 mr-2" />Créer le client
              </Button>
            )
          )}
        </Card>
      ...
      <Card className="p-5 space-y-3">
        ...
        <Textarea rows={5} placeholder="Notes de suivi, échanges, relances…"
          value={notes} onChange={(e) => setNotes(e.target.value)} disabled={!canWrite} />
        {canWrite && (
        ...
        {demande.statut === "perdue" && demande.raison_perte && (
          <div className="text-sm bg-destructive/5 border border-destructive/20 rounded-md p-3">
            <span className="font-medium">Raison de perte : </span>{demande.raison_perte}
          </div>
        )}
      </Card>
          ...
          <div className="flex flex-wrap gap-2">
            {demande.statut !== "en_cours" && (
              <Button variant="outline" size="sm" onClick={() => updateStatut("en_cours")}>
                <PlayCircle className="h-4 w-4 mr-2" />Marquer en cours
              </Button>
            )}
            ...
            <Dialog open={perteOpen} onOpenChange={setPerteOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="text-destructive">
                  <XCircle className="h-4 w-4 mr-2" />Marquer perdue
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Raison de perte</DialogTitle></DialogHeader>
                <Textarea rows={3} placeholder="Prix trop élevé, délai trop court…"
                  value={raisonPerte} onChange={(e) => setRaisonPerte(e.target.value)} />
                <DialogFooter>
                  <Button variant="outline" onClick={() => setPerteOpen(false)}>Annuler</Button>
                  <Button onClick={async () => { await updateStatut("perdue", raisonPerte); setPerteOpen(false); }}>
                    Confirmer
                  </Button>
                </DialogFooter>
              </DialogContent>
            ...
            <Dialog open={transformOpen} onOpenChange={setTransformOpen}>
              <DialogTrigger asChild>
                <Button size="sm">
                  <Sparkles className="h-4 w-4 mr-2" />Créer une cotation
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Transformer en cotation</DialogTitle></DialogHeader>
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => setTransformOpen(false)}>Annuler</Button>
