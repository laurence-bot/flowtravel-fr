...
const RECUP_STATUT_LABELS: Record<RecupDemande["statut"], string> = {
  demande: "En attente",
  approuvee: "Approuvée",
  refusee: "Refusée",
  annulee: "Annulée",
...
function CongesPage() {
  ...
  const submit = async () => {
    ...
    try {
      await createAbsence({ employee_id: emp.id, ...form });
      setOpen(false);
      setForm({ type: "conge_paye", date_debut: "", date_fin: "", motif: "" });
      load();
  ...
  const submitRecup = async () => {
    if (!emp) return;
    if (!recupForm.date_souhaitee) {
      toast.error("Date souhaitée requise");
      return;
    }
    if (!recupForm.heure_debut || !recupForm.heure_fin) {
      toast.error("Heures requises");
      return;
    }
    if (recupHeures <= 0) {
  ...
  // soldes simples
  const taken = (t: AbsenceType) =>
    items
      .filter((a) => a.type === t && (a.statut === "approuvee" || a.statut === "signee"))
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <Card className="p-0 overflow-hidden overflow-x-auto">
        {items.length === 0 ? (
          <div className="p-10 text-center text-muted-foreground">Aucune demande</div>
        ) : (
              ...
              {items.map((a) => (
                <tr key={a.id} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-muted">{ABSENCE_STATUT_LABELS[a.statut]}</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {recups.length > 0 && (
        <Card className="p-0 overflow-hidden overflow-x-auto">
          <div className="px-4 py-3 border-b bg-muted/40 text-xs uppercase text-muted-foreground">
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead className="bg-muted/20 text-xs uppercase text-muted-foreground">
              <tr>
                ...
                <th className="text-left px-4 py-2">Horaires</th>
                <th className="text-left px-4 py-2">Heures</th>
            ...
            <tbody>
              {recups.map((r) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    {r.heure_debut && r.heure_fin ? `${r.heure_debut.slice(0, 5)}–${r.heure_fin.slice(0, 5)}` : "—"}
                  </td>
                  <td className="px-4 py-2">{r.heures_demandees}h</td>
                  <td className="px-4 py-2 max-w-[260px] truncate">{r.motif ?? "—"}</td>
                  <td className="px-4 py-2">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-muted">{RECUP_STATUT_LABELS[r.statut]}</span>
                  </td>
                  <td className="px-4 py-2 text-right">
                    {r.statut === "demande" && (
                      <Button size="sm" variant="ghost" onClick={() => cancelRecup(r.id)}>
                        Annuler
                      </Button>
                    )}
                  </td>
          ...
          <div className="grid gap-3">
            ...
            {form.date_debut && form.date_fin && (
              <p className="text-xs text-muted-foreground">
                {computeWorkingDays(form.date_debut, form.date_fin)} jours ouvrés
              </p>
            )}
            <div>
      ...
      <Dialog open={recupOpen} onOpenChange={setRecupOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Demander une récupération</DialogTitle>
          </DialogHeader>
          <div className="grid gap-3">
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Ex : rendez-vous médical, démarches administratives…"
              />
            </div>
          </div>
          <DialogFooter>
