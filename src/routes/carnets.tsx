      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Créez un carnet interactif pour vos clients (planning, photos, infos pratiques).
          </p>
        </div>
          ...
          <TableBody>
            ...
            {list.map((c) => (
              <TableRow key={c.id}>
                ...
                <TableCell>
                  <Badge variant={c.statut === "publie" ? "default" : "secondary"}>
                  ...
                  {c.statut === "publie" && (
                    <>
                      <Button size="sm" variant="ghost" onClick={() => copy(c.token)}>
                        {copied === c.token ? <Check className="w-3 h-3 mr-1" /> : <Copy className="w-3 h-3 mr-1" />}
                        Lien
                      </Button>
                      <Button size="sm" variant="ghost" asChild>
                        <a href={`/carnet/${c.token}`} target="_blank" rel="noreferrer">
...
function CarnetCreateDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Créer un carnet de voyage</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-2">
          <div>
            ...
            <Input
              ...
              placeholder="ex. Voyage de Marie au Japon"
            />
          </div>
          <div>
        ...
        <DialogFooter>
          <Button onClick={() => onCreate(titre, dossierId, destination)} disabled={!titre.trim()}>
            Créer
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Éditer « {carnet.titre} »</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-2">
          <div>
            <Label>Titre</Label>
          ...
          <div className="border-t pt-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => setJours([...jours, { titre: `Jour ${jours.length + 1}` }])}
              >
              ...
              {jours.map((j, i) => (
                <Card key={i} className="p-3 space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder="Description / activités…"
                    value={j.description ?? ""}
                    onChange={(e) => {
                      const n = [...jours];
                  ...
                  />
                  <Button size="sm" variant="ghost" onClick={() => setJours(jours.filter((_, k) => k !== i))}>
                    Supprimer
                  </Button>
                </Card>
              ))}
