...
export const Route = createFileRoute("/mon-espace/evaluation")({
  ...
  component: () => <AppLayout><MyEval /></AppLayout>,
});
...
function MyEval() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success("Auto-évaluation enregistrée"); load();
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {!active ? <Card className="p-10 text-center text-muted-foreground">Aucune campagne en cours</Card> :
        <Card className="p-6 space-y-4">
          <div><div className="text-xs uppercase text-muted-foreground">Année</div><div className="text-xl font-display">{active.annee}</div></div>
          <div>
            <Label>Auto-évaluation</Label>
            <Textarea rows={10} value={autoEval} onChange={(e) => setAutoEval(e.target.value)} placeholder="Atteinte des objectifs, points forts, axes d'amélioration, formations souhaitées…" />
          </div>
