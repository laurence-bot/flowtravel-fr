              ...
              {items.map((c) => (
                <tr key={c.id} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-muted">{c.statut}</span>
                  </td>
                  <td className="px-4 py-2 text-right">
                    {c.statut === "a_signer" && (
                      <a href={`/contrat-signer/${c.token}`} className="text-blue-600 hover:underline text-xs">
                        Signer →
                      </a>
                    )}
                    {c.statut === "signe" && (
                      <span className="text-xs text-green-700">Signé le {c.signed_at?.slice(0, 10)}</span>
                    )}
                  </td>
                </tr>
              ))}
