...
// Cotation Sophie Marchand · Japon · 14 jours
const LIGNES = [
...
export const Scene3Cotation: React.FC<{ format: Format }> = () => {
  ...
  // Animation : la marge se calcule en temps réel
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.7fr 1fr", gap: 16 }}>
          ...
          <FTCard style={{ padding: 0, overflow: "hidden" }}>
            <div style={{
              ...
              fontWeight: 600,
              display: "grid",
              gridTemplateColumns: "0.7fr 2.2fr 0.6fr 0.7fr 0.7fr",
              gap: 10,
            }}>
              <div>Type</div>
              <div>Prestation</div>
              <div>Devise</div>
              <div style={{ textAlign: "right" }}>Achat</div>
            ...
            {LIGNES.map((l, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <div>
                    <FTBadge variant="default">{l.type}</FTBadge>
                  </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <FTCard style={{ background: `linear-gradient(135deg, ${COLORS.bg} 0%, ${COLORS.gold}20 100%)` }}>
              ...
              <div style={{ fontFamily: FONT_DISPLAY, fontSize: 36, color: COLORS.text, marginTop: 4, fontWeight: 500 }}>
                {venteAnim.toLocaleString("fr-FR")} €
              </div>
              <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 2 }}>
                Acompte 30% · {Math.round(venteAnim * 0.3).toLocaleString("fr-FR")} €
              </div>
            </FTCard>
            ...
            <FTCard style={{ background: `linear-gradient(135deg, ${COLORS.bg} 0%, ${COLORS.olive}20 100%)`, border: `1px solid ${COLORS.olive}50` }}>
              ...
              <div style={{ fontFamily: FONT_DISPLAY, fontSize: 36, color: COLORS.olive, marginTop: 4, fontWeight: 500 }}>
                {margeAnim.toLocaleString("fr-FR")} €
              </div>
              <div style={{ fontSize: 11, color: COLORS.olive, marginTop: 2, fontWeight: 600 }}>
                {pctAnim.toFixed(1)}% du CA
              </div>
            ...
            <FTCard>
              <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.15em", color: COLORS.textMuted, fontWeight: 600, marginBottom: 6 }}>
                Recalcul temps réel
              </div>
              <div style={{ fontSize: 11, color: COLORS.text, lineHeight: 1.5 }}>
                Chaque ligne ajustée → marge mise à jour instantanément. Multi-devises géré en natif.
              </div>
            </FTCard>
          </div>
        </div>
