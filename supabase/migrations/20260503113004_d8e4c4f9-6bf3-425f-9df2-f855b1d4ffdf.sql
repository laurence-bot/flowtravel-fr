...
ALTER TABLE public.agences
  ADD COLUMN IF NOT EXISTS est_etablissement_secondaire boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS siren_siege text;

-- 2) Flag FX agence
ALTER TABLE public.agency_settings
  ADD COLUMN IF NOT EXISTS utilise_couvertures_fx boolean NOT NULL DEFAULT false;

-- 3) Conversion atomique cotation -> dossier
...
DECLARE
  ...
  pct numeric;
BEGIN
  -- Charger la cotation (la RLS filtre déjà sur user_id = auth.uid())
  ...
  -- Pour chaque ligne, créer la facture fournisseur prévisionnelle + échéances
  FOR l IN
  ...
  LOOP
    ...
    INSERT INTO public.factures_fournisseurs (
      ...
      cot.user_id, dossier_id, l.fournisseur_id,
    ...
    -- Échéances ligne (acomptes + solde) — uniquement celles dont le pct > 0
    ech_pcts := ARRAY[
    ...
    FOR i IN 1..4 LOOP
      ...
      IF pct > 0 THEN
        INSERT INTO public.facture_echeances (
          ...
          cot.user_id, facture_id, i, ech_types[i]::echeance_type, 'a_payer',
          l.devise,
  ...
  UPDATE public.cotations
     SET statut = 'transformee_en_dossier', dossier_id = dossier_id
   WHERE id = cot.id;
...
GRANT EXECUTE ON FUNCTION public.transformer_cotation_en_dossier(uuid) TO authenticated;